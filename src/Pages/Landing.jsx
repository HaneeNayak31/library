...
function Landing() {
  ...
  return (
    <div className="space-y-14 pt-5 md:space-y-16 bg-white"> 
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-8">
          {newArrivals.map((book, idx) => (
            <div key={idx} className="group perspective-1000">
              ...
              <div className="relative transition-transform duration-500 group-hover:-translate-y-3">
                ...
                <div className="aspect-3/4 rounded-xl overflow-hidden shadow-lg border border-slate-100 relative">
                  ...
                  <div className="absolute top-4 left-4">
                    <span className="px-3 py-1 bg-black/50 backdrop-blur-sm rounded-full text-[9px] font-bold text-white uppercase tracking-tighter">
                      {book.tag}
                    </span>
                  </div>
                </div>
      ...
      {/* --- SECTION 3: QUOTE --- */}
      <section className="py-6 md:py-8 border-y border-slate-100 bg-[#FDFDFB]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {[
            {  label: "Schedule", primary: "10:30 AM - 6:10 PM", secondary: "Mon - Sat", accent: "bg-blue-500" },
            {  label: "Resources", primary: "25,000+ Print Items", secondary: "Digital Access", accent: "bg-amber-500" },
            {  label: "Assistance", primary: "Research Support", secondary: "Expert Librarians", accent: "bg-emerald-500" }
          ].map((item, i) => (
            <div key={i} className="group relative bg-white border border-slate-100 p-6 rounded-4xl shadow-sm hover:shadow-xl hover:shadow-slate-200/50 transition-all duration-500 hover:-translate-y-1 overflow-hidden">
              ...
              <div className="flex items-center gap-5 relative z-10">
                <div className={`flex-none w-14 h-14 rounded-2xl ${item.accent} bg-opacity-10 flex items-center justify-center transition-all duration-500  group-hover:rotate-180`}>
      ...
      <section className="relative py-14 md:py-16 flex flex-col items-center justify-center">
        
        
        <div className="absolute inset-0 flex items-center justify-center opacity-20 pointer-events-none">
          <div className="w-80 h-80 bg-blue-100 rounded-full blur-xl"></div>
        </div>

        <div className="relative z-10 text-center space-y-1">
          <span className="text-[10px] font-black uppercase tracking-[0.6em] text-black mb-4 block">The Visitor Pulse</span>

          <h2 className="text-7xl md:text-9xl font-serif tracking-tighter text-slate-900 tabular-nums">
            {visitorCount.toLocaleString()}
          </h2>

          <div className="h-px w-16 bg-blue-100 mt-6"></div>

          <p className="max-w-md mx-auto text-[13px] text-slate-500 italic">
